...
import (
	...
	"github.com/submaline/services/gen/user/v1/userv1connect"
...
func main() {
	...
	addr := fmt.Sprintf("0.0.0.0:%v", os.Getenv("USER_SERVICE_PORT"))
	...
	if err := http.ListenAndServeTLS(
		addr,
		fmt.Sprintf("/data/ssl_certs/%s/staging/domain.csr", os.Getenv("USER_DOMAIN")),
		fmt.Sprintf("/data/ssl_certs/%s/staging/domain.key", os.Getenv("USER_DOMAIN")),
		h2c.NewHandler(mux, &http2.Server{}),
	); err != nil {
		log.Fatal(err)
	}
